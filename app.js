...
//gioapify to reverse geocoding
const weather = {
  API: "a5810fc9d2b5de67b5415fac0cf6c07e",
  fetchLat: function (lat, long) {
    fetch(
      `https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${long}&apiKey=a26b661efecb442796247bd8a7de99f1`
    )
      .then((response) => response.json())
      .then((result) =>
        weather.fetchWeather(result.features[0].properties.city)
      )
      ...
      .then((res) => {
        if (res.ok) {
          ...
          alert(
            res.status +
              " Not found." +
              " Please Enter Correct Spelling Of City or State or Country."
          );
        }
      })
...
searchButton.addEventListener("click", () => {
  if (!inputField.value.trim()) {
    return alert("Please Enter Your City or State or Country");
  }
...
inputField.addEventListener("keyup", (event) => {
    ...
    if(!inputField.value.trim()) return alert("Please Enter Your City or State or Country");
    return weather.search()
});
